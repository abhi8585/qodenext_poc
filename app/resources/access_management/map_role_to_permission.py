...
class MapRoleToPermissionResource(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            if not role_id or not permission_id :
                return {'status' : 200, 'message' : 'Permission and Role id is required', 'data' : []}

            if role_id and permission_id:
                row_obj = dict(role_id=role_id, permission_id=permission_id)
                insert_mapped_status = mysql_client.insert(table_name='role_to_permission',column_values=row_obj)
                if insert_mapped_status:
                    mapped_status = dict(status=200,data=insert_mapped_status)
                else:
                    logger.log_info(f"Error while mapping permission with role")
                return mapped_status
        except Exception as e:
            logger.log_error(f"Error while Mapping Role To Permission, Error : {e}")
            return mapped_status
