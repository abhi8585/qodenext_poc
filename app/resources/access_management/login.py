...
class LoginResource(Resource):
    ...
    def post(self):
        try:
            ...
            user = mysql_client.select('user', filter_condition=f"WHERE user_email = '{email}'")
            if not user:
            ...
            if len(user['results']) > 0:
                ...
                if role_id:
                    if len(role_id['results']) > 0:
                        ...
                        role_name = mysql_client.select(table_name='role',filter_condition=f"where role_id = {role_id}")
                        if role_name:
                            if len(role_name) > 0:
                                role_name = role_name['results'][0]['role_name']
                        else:
                            print(f"role name is empty")
                else:
                    role_name = ''
            ...
            logger.log_error(f"An error occurred during login: {str(e)}")
